...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  scope_id_required: "특정 활동·기구로 범위를 지정하려면 ID를 입력해야 합니다.",
  ...
  return (
    <section>
      ...
      <form method="POST" action="/api/admin/roles/grant" style={{ marginBottom: 24 }}>
        ...
        <label htmlFor="scopeType" style={{ display: "block", fontSize: 14, marginBottom: 4 }}>
          범위
        </label>
        <select
          id="scopeType"
          name="scopeType"
          defaultValue={ScopeType.GLOBAL}
          style={{ width: "100%", padding: 10, fontSize: 16, marginBottom: 12 }}
        >
        ...
        <label htmlFor="scopeId" style={{ display: "block", fontSize: 14, marginBottom: 4 }}>
          활동·기구 ID — 범위를 "전체"로 두면 비워둡니다
        </label>
        <input
          id="scopeId"
          name="scopeId"
              ...
              <ul style={{ listStyle: "none", padding: 0, marginTop: 4 }}>
                {account.permissionGrants.map((grant) => (
                  <li key={grant.id} style={{ fontSize: 14, marginBottom: 4 }}>
                    ...
                    {grant.scopeType !== "GLOBAL" && ` (${SCOPE_TYPE_LABELS[grant.scopeType]}: ${grant.scopeId})`}
