...
export async function POST(request: Request) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.need.updateMany({
      where: { raisedBySubjectId: source.id },
      data: { raisedBySubjectId: target.id },
    });
    await tx.need.updateMany({
      where: { beneficiarySubjectId: source.id },
      data: { beneficiarySubjectId: target.id },
    });
    await tx.activity.updateMany({
      where: { organizerSubjectId: source.id },
      data: { organizerSubjectId: target.id },
    });
    ...
    await tx.contribution.updateMany({
      where: { contributorSubjectId: source.id },
      data: { contributorSubjectId: target.id },
    });
    await tx.permissionGrant.updateMany({
      where: { scopeType: "ORG_UNIT", scopeId: source.id },
      data: { scopeId: target.id },
    });
    if (sourceAccount) {
