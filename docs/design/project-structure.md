  ...
  app/
    ...
    (protected)/          로그인(및 필요 시 TOTP)이 끝나야 들어오는 화면 — layout.tsx가 막는다
      ...
      activities/
        ...
        [activityId]/lineage/page.tsx 정정 계보 전체 보기 — 원본부터 최신까지 모든 버전을 순서대로
      my/
...
- **로그인 요청·2단계 인증 속도 제한(`src/lib/rate-limit.ts`)**: Redis 없이
  ...
  (scope, key) 조합의 최근 시도 수를 세고, 허용될 때만 새 행을 남기면서 같은
  조합의 창(window) 밖 오래된 행을 지운다 — 표가 무한정 커지지 않는다.
  - `login_request`: 이메일 문자열(소문자 정규화) 자체가 키다. 계정 존재 여부
    확인보다 먼저 걸려, 존재하지 않는 이메일도 똑같이 제한받는다(계정 유무가
    새어나가지 않게 하려는 기존 설계와 일관). 15분에 5회, 초과 시
    `/login?error=rate_limited`.
  - `totp_verify`: 계정 ID가 키다. 6자리 코드든 복구 코드든 같은
    `/api/auth/totp/verify` 창구를 거치므로 하나의 제한으로 둘 다 막는다.
    15분에 10회, 초과 시 코드가 맞아도 통과시키지 않고
    `/login/totp?error=rate_limited`로 되돌린다 — "허용 한도 안에서 계속
    틀리기"로 우회할 수 없도록, 검증 결과와 무관하게 매 시도가 카운트된다.
...
- **정직하게 남겨둔 것**: v0.1이 요구하는 "종료 체크"(열린 청구·지급 처리, 후속 과업
  ...
  지금의 종료 평가는 자유 텍스트 한 줄로만 남는다.

## 활동 결재 위임 (`Activity.approverAccountId`, v0.1 §4.10 구현)

"준비 승인"의 결재 권한자를 책임자와 분리한다 — 지금까지 없던 위임 규정을
채운다("아직 없는 것"에 있던 틈).

- **한 줄짜리 필드로 충분했다**: 새 역할·권한 체계를 만들지 않고
  `Activity.approverAccountId`(선택, 다른 `...AccountId` 필드들과 같은 원칙으로
  Prisma 관계가 아니라 순수 문자열 참조) 하나로 해결했다. 비어 있으면 지금까지처럼
  책임자 본인이 승인·반려까지 처리한다 — 기존 활동은 전부 이 상태라 회귀가 없다.
- **위임은 오직 승인·반려에만 적용된다**: `approverAccountId`가 채워져 있고 지금
  `PENDING_APPROVAL` 상태일 때만, 승인·반려 두 전이의 처리 자격이 위임받은 계정으로
  넘어간다. 승인 요청·시작·수행완료·종료·보류·재개·취소 등 나머지 전이는 위임 여부와
  무관하게 여전히 책임자만 할 수 있다 — "결재만" 분리하고 활동 운영 전체를 넘기는
  것이 아니기 때문이다.
- **자기 결재를 막는다**: 위임이 걸려 있으면 책임자 본인은 그 활동을 승인·반려할
  수 없다(`error=forbidden`) — 위임을 "참고용 알림"이 아니라 실제 권한 분리로
  만들기 위함이다. 상세 화면에서도 책임자에게는 "이 활동은 OOO에게 결재를
  위임했습니다. 승인·반려 대기 중입니다"라는 안내만 보이고 승인·반려 버튼
  자체가 사라진다 — 위임받은 계정에게만 그 버튼이 보인다.
- **위임 대상은 활동 수정 화면(`/activities/[id]/edit`)에서 지정한다**: 새 화면을
  만들지 않고 이미 있는 수정 폼에 선택형 하나를 추가했다. 책임자 본인은 위임
  대상 후보에서 빠진다(자기 자신에게 위임하는 건 위임이 아니므로) — 그래도 관리자
  화면 조작 등으로 책임자와 같은 값이 넘어오면 API가 다시 확인해
  `error=approver_same_as_manager`로 거부한다.
- **정직하게 남겨둔 것**: 위임은 활동 하나하나에 개별 설정한다 — "이 사람의 모든
  활동은 항상 아무개가 결재한다" 같은 조합원 단위 상시 위임 규칙은 없다. 결재
  위임 이력 자체를 감사기록으로 조회하는 전용 화면도 없다(활동 수정
  `AuditLog`의 `approverAccountId` 변경분으로만 남는다 — `/admin/audit-log`·
  `/my/audit-log`에서 다른 필드 변경과 함께 보인다).
...
- **거듭 정정은 최신 버전에서만**: 이미 정정된 원본에서 다시 "정정 이력
  ...
  시작해야 한다.
- **정정 계보 전체 보기(`/activities/[activityId]/lineage`)**: 배너는 원본↔최신
  한 단계만 오갈 수 있어, 두 번 이상 정정된 활동은 중간 버전을 볼 방법이
  없었다. `src/lib/activity-revision-lineage.ts`의 `getActivityRevisionLineage()`가
  `revisionOfId`를 거슬러 원본까지 올라간 뒤 `supersededByActivityId`를 따라
  최신까지 다시 내려오는 방식으로 전체 계보를 한 번에 모은다(방문한 id를
  추적해 데이터 이상으로 인한 순환에도 무한 루프에 빠지지 않는다). 계보의
  어느 버전에서 봐도 같은 목록이 나오고, 지금 보고 있는 버전에는 "지금 보는
  버전" 표시가 붙어 자기 자신으로는 링크를 걸지 않는다. 활동 상세 화면과
  같은 접근 기준(로그인만 하면 누구나 조회 가능)이며, 원본이거나 정정본인
...
attachment-storage.ts` 두 함수(저장·읽기)만 바꾸면 되도록 감싸 뒀고, 실제로 그
이전 코드 경로도 이제 있다("첨부파일 저장소 — S3/MinIO 지원" 절 참고). 기여
...
- **정직하게 남겨둔 것**: ADR-0004가 명시하듯 이 저장 방식은 서버를 한 대만
  ...
  않는다. (수평 확장·백업 문제는 아래 "첨부파일 저장소 — S3/MinIO 지원"으로
  옮기면 해결되지만, 옮기기 전까지는 여전히 이 전제 그대로다.)

## 첨부파일 저장소 — S3/MinIO 지원 (`src/lib/attachment-storage.ts`)

ADR-0004가 약속한 "나중에 S3/MinIO로 옮길 때는 이 파일 하나만 바꾸면 된다"를
실제로 지켰다 — 바꾼 게 아니라, 그 파일 **안에** 두 번째 저장 방식을 추가하고
환경변수로 고르게 했다. 업로드·다운로드 API 네 곳(활동·상담·수요·기여 첨부)은
이 파일이 내보내는 함수 이름과 시그니처가 그대로라 한 줄도 손대지 않았다.

- **환경변수 하나로 전환**: `ATTACHMENT_STORAGE_DRIVER`가 `"s3"`면 AWS SDK
  (`@aws-sdk/client-s3`)로 업로드·다운로드하고, 그 외(빈 값 포함, 기본값)에는
  지금까지의 로컬 디스크 코드를 그대로 쓴다. 아무 것도 설정하지 않은 기존
  배포는 동작이 전혀 바뀌지 않는다 — 새 배포에만 해당하는 선택 사항이다.
- **MinIO 등 자체 호스팅 S3 호환 서비스도 그대로 지원한다**: AWS S3 전용이
  아니라 `ATTACHMENT_S3_ENDPOINT`(예: `http://minio.internal:9000`)와
  `ATTACHMENT_S3_FORCE_PATH_STYLE=true`를 채우면 같은 코드로 MinIO 등에
  붙는다 — MinIO는 버추얼호스트 스타일 URL(`bucket.host/key`)을 지원하지 않는
  경우가 많아, path-style(`host/bucket/key`)을 강제하는 옵션이 따로 필요하다.
  자격 증명은 새 전용 환경변수를 만들지 않고 AWS SDK가 원래 읽는
  `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`(또는 IAM 역할 등 SDK 기본
  자격 증명 체인)를 그대로 쓴다.
- **DB의 `fileKey`는 드라이버와 무관하게 같은 값**: `generateAttachmentFileKey()`가
  만드는 무작위 키(로컬 파일명이자 S3 객체 키)는 두 드라이버에서 형식이 똑같다
  — 드라이버를 나중에 전환해도 기존 `Attachment` 레코드의 `fileKey`를 고칠
  필요가 없다(다만 그 시점까지 쌓인 실제 파일 자체를 새 버킷으로 옮기는 일은
  별도다 — 아래 "정직하게 남겨둔 것" 참고).
- **정직하게 남겨둔 것**: 실제 AWS S3나 실제 MinIO 서버가 아니라, 같은 S3 REST
  API를 구현한 로컬 목(mock) 서버(`s3rver`, npm 패키지, Docker 없이 실행)를
  띄워 놓고 실제 Next.js 서버 + 실제 HTTP 업로드·다운로드 API를 통해
  검증했다 — AWS SDK가 그 목 서버에 보낸 요청·응답은 진짜 S3와 같은 프로토콜을
  쓰므로 신뢰할 만한 검증이지만, 실제 AWS 계정 요금·리전 지연·IAM 정책 같은
  것까지 검증한 것은 아니다. 지금까지 로컬 디스크에 쌓인 첨부파일을 새 버킷으로
  옮기는 이관 스크립트도 없다("아직 없는 것" 참고) — 조합이 실제로 버킷을
  정하는 시점에 그때 쌓인 파일 개수만큼만 한 번 처리하면 되는 일이라 미리
  만들지 않았다.
...
- **후속 필요 생성·접수 후 수정은 이후 별도 화면으로 구현했다**: 종결 후
  "새 요청은 원본을 참조하는 새 필요로 만든다"(v0.1)는 "상담·수요 후속 필요
  생성" 절에서, 접수 후 내용 수정은 "상담·수요 수정 화면" 절에서 각각 다룬다 —
  이 절을 처음 쓸 때는 둘 다 없었지만 지금은 있다.
...
### 계정 상태 관리 화면 (`/admin/accounts`, FR-01 구현)

관리자 허브에 마지막까지 남아 있던 자리표시자 화면(FR-01 "계정 상태")을 채웠다.
그동안 계정을 정지·탈퇴 처리하려면 DB를 직접 만져야 했다. 활동·상담 상태표와
같은 방식으로 상태표 하나(`src/lib/account-status.ts`)에 "어느 상태에서 어느
동작이 가능한지"·"사유가 필수인지"를 선언한다(`activate`/`suspend`/
`reactivate`/`withdraw`/`rejoin`).

- **세션을 따로 폐기할 필요가 없다**: `getActiveSession()`(session.ts)이 매
  요청마다 `account.status !== "ACTIVE"`를 확인하고 있었으므로(인증 흐름
  구현 때부터 있던 코드), 이 화면은 `Account.status`만 바꾸면 된다 — 이미
  로그인된 세션 쿠키가 남아 있어도 그 다음 요청부터 바로 막히거나(정지·탈퇴)
  풀린다(재활성화·재가입). 실제로 정지 전에 발급받은 세션 쿠키로 테스트해
  확인했다(아래 검증 이력 참고).
- **자기 자신은 절대 못 바꾼다**: 역할 종료 화면은 "본인이 마지막 관리자
  권한을 스스로 회수하는 것"만 막는(남은 관리자가 있으면 허용) 상대적
  규칙이었지만, 계정 상태는 그 사람의 접근 전체를 막는 것이라 다른 관리자가
  남아 있는지와 무관하게 자기 자신에게는 아예 적용할 수 없게 했다
  (`error=cannot_change_self`).
- **정지·탈퇴만 사유가 필수다**: 제한 상태로 들어가는 동작(`suspend`,
  `withdraw`)은 사유를 반드시 남기지만, 그 반대(`reactivate`, `rejoin`)는
  기구 복원과 같은 이유로 사유를 요구하지 않는다 — 제한을 푸는 쪽은 "긍정적"
  결정으로 본다.
- **PENDING→ACTIVE(`activate`)는 지금 실제로 쓰이지 않는다**: 초대 수락 시
  계정을 항상 바로 ACTIVE로 만들기 때문에(코드 전체를 확인함) 스키마 기본값
  (`@default(PENDING)`)에 실제로 도달하는 경로가 없다. 그래도 스키마와 맞춰
  전이표에 넣어 두어, 나중에 그런 계정이 생기더라도 이 화면에서 바로 처리할
  수 있게 방어적으로 남겼다.
- 모든 전이는 `AuditLog`(`entityType: "Account"`, `action: "STATUS_CHANGE"`)에
  행위자·이전/이후 상태·사유와 함께 남아 `/admin/audit-log`에서 조회된다.
...
- **세 묶음으로 구성**: "참여 중인 활동"은 `ActivityAssignment.status`가
  ...
  - **보완 요청 카드마다 재제출 링크**: 보완 요청된 기여 각각에 사유(빨간 글자)와
    ...
    한 것이다.
- **역할에 따라 추가 섹션이 얹힌다(v0.1 §4.2/4.5 구현)**: 조합원 기본 구성(위
  세 묶음)은 누구나 똑같이 보되, 그 위에 역할별 요약 카드를 조건부로 얹는다.
  둘 다 "보여줄 게 있을 때만" 나타난다 — 자격은 있지만 처리할 항목이 0건이면
  섹션 자체가 안 뜬다(빈 카드로 화면을 채우지 않기 위함).
  - **담당자로서 처리할 일**: `canAccessReviewInbox`(역할이 있거나 실제 담당
    활동·상담이 있음 — `/review` 접근 자격과 같은 판정)가 있으면, 확인 대기
    기여(`/review`와 정확히 같은 조건: `status=SUBMITTED`이고 본인이 관리하는
    활동·담당하는 상담), 본인이 관리하는 활동의 참여 신청 대기
    (`ActivityAssignment.status=PROPOSED`), 본인이 담당하는 상담·수요 중
    확인을 시작하지 않은 건(`status=RECEIVED`) 세 가지를 건수와 함께 보여주고
    각각 해당 화면으로 링크한다.
  - **운영 현황**: SECRETARIAT·SYSTEM_ADMIN 역할이 있으면, 조합 전체 기준으로
    응답 대기 중인 초대(`status=PENDING`)와 아직 확인을 시작하지 않은
    상담·수요(`status=RECEIVED`, 담당자 무관 전체 건수)를 보여준다 — 위
    "담당자로서 처리할 일"이 "내가 담당하는 것"만 보는 것과 달리, 이건
    "조합 전체에서 새로 들어와 아무도 아직 손대지 않은 것"을 보는 사무국
    관점이다(v0.1 §4.2 "미응답 필요").
  - **정직하게 남겨둔 것**: v0.1 §4.5가 그리는 사업 책임자 화면의 예산·청구·
    성과 탭, §4.2의 마감·지연 청구 등은 계약·청구·성과 지표 데이터가 R1
    스키마에 없어 옮기지 못했다 — 지금은 이미 있는 화면으로 이어주는
    요약 카드로 범위를 좁혔다.
...
## 상담·수요 후속 필요 생성 (v0.1 §3.1 구현)

v0.1이 명시한 규칙: "종결 후 새 요청은 원본을 참조하는 새 필요로 만든다. 오입력
정정은 권한자만 이력과 함께 재개한다." 뒤 문장(오입력 정정)은 이미 위 수정
화면이 처리하고, 앞 문장(종결 후 새 요청)이 이번에 채운 틈이다.

- **정정이 아니라 새 기록이다**: 활동·기여의 `revisionOfId`는 원본을 "대체"한다
  (원본은 `supersededBy...`로 낡은 기록이 되어 실사용 UI에서 숨는다). 상담·수요의
  `followsUpOnNeedId`는 그렇지 않다 — 원본과 후속 둘 다 독립된 현재 기록으로
  남는다. 종결된 상담이 "틀려서" 새로 만드는 게 아니라, 같은 맥락에서 "또
  요청이 들어와서" 새로 만드는 것이기 때문이다(원인이 다르면 스키마 의미도
  다르다).
- **원본은 종결(CLOSED)된 것만 가능**: `/needs/new`의 "이전 상담·수요의
  후속인가요?" 선택형은 `status: CLOSED`인 상담·수요만 보여주고, API도 같은
  조건을 다시 확인해 `error=invalid_follow_up`으로 거부한다. 사업화(`CONVERTED`)는
  v0.1이 "완료 성과가 아니라 수요가 실행으로 넘어간 상태"라고 못박은 별개
  상태라 후속의 원본이 될 수 없다 — 사업화된 필요는 `NeedActivityLink`로 이미
  활동에 연결돼 있으므로, 추가 요청은 그 활동에 배정을 늘리는 문제이지 새
  상담을 만들 문제가 아니다.
- **양방향으로 보여준다**: 후속 상담·수요의 상세 화면에는 "이 상담·수요는
  [원본]의 후속입니다" 배너가, 원본(종결된 건)의 상세 화면에는 "후속
  상담·수요" 목록(상태 배지 포함, 여러 건 가능 — 1:N)이 뜬다. 종결된 상담·수요를
  보는 화면(활동 운영 역할이 있는 사람 누구나, 담당자로 한정하지 않음)에는
  "후속 상담·수요 만들기 →"가 `/needs/new?followsUpOnNeedId=<id>`로 링크돼
  선택형이 미리 채워진 채 열린다.
- **다른 필드는 자동으로 옮기지 않는다**: 제기한 사람·대상(수혜)·예산 등은
  후속이라고 자동 복사하지 않는다 — 같은 맥락이라도 요청자나 대상이 바뀌었을
  수 있어, 잘못 넘겨짚는 것보다 담당자가 다시 확인해 입력하는 편이 안전하다고
  판단했다.
...
- **정직하게 남겨둔 것**: 이미지를 레지스트리에 올리거나 실제 서버에 배포하는
  ...
  "Docker 이미지 레지스트리 배포" 참고).
- **후속 업데이트 — 실제 GitHub Actions에서 확인함**: 이 절을 처음 쓸 때는
  워크플로 자체를 GitHub Actions 러너에서 실행해 본 적이 없어(`act` 같은
  로컬 러너가 이 샌드박스에 없다) 문법만 YAML 파서로 검증한 상태였다. 이후
  실제 PR(#2)을 올려 두 잡을 처음 실행해 보니, `docker` 잡이 `runner`
  스테이지의 `COPY --from=builder /app/public ./public`이 존재하지 않는
  `public/`을 복사하려 한 버그를 즉시 잡아냈다(이 저장소에는 정적 자산이
  없어 `public/` 디렉터리 자체가 없다 — 이 샌드박스는 Docker Hub 접근이
  막혀 있어 로컬에서는 발견할 수 없었던 문제). 그 COPY 줄을 지우고 다시
  푸시하자 `app`·`docker` 두 잡 모두 성공했다.

## 데이터 백업·내보내기 (`/admin/backup`, `/my/profile`, 병행운영전략 v0.2 구현)

병행운영전략 v0.2가 "v0.1 참여 시범" 범위로 명시한 항목(140행) 중 "내보내기·백업"이
지금까지 유일하게 빠져 있었다 — `/admin` 화면에도 "백업 상태… 아직 구현되지
않았습니다"라는 자리표시자가 처음부터 있었다. 두 갈래로 나눠 구현했다.

- **관리자 전체 백업(`/admin/backup`)**: SECRETARIAT/SYSTEM_ADMIN만(`/admin`의
  다른 화면과 같은 권한). "전체 데이터 내보내기" 버튼을 누르면 사람(Subject)·
  계정·분류·상담/수요·활동·배정·기여·첨부파일 메타데이터·알림·권한부여·초대·
  감사기록을 테이블별 배열로 묶은 JSON 파일 하나를 내려받는다. 병행운영전략
  23행 "데이터와 소스코드를 조합이 통제하며, 내보내기와 복원으로 다른 운영자로
  이전할 수 있게 한다"를 만족하기 위한 것 — 재해 복구용 바이너리 DB 덤프가
  아니라 업무 데이터를 사람이 읽고 다른 시스템으로 옮길 수 있는 구조화된 형태다.
- **본인 데이터 내보내기(`/my/profile`의 "내 데이터 내보내기" 버튼)**: 역할과
  무관하게 로그인한 계정이면 누구나 자신에 관한 데이터(계정·사람 정보·본인
  기여·참여 배정·책임 활동·담당 상담/수요·본인이 접수/수혜자인 상담/수요·알림·
  권한부여·업로드한 첨부·본인이 행위자인 감사기록)를 JSON으로 받는다 — 개인
  정보 자기결정권 쪽 요구를 채운다.
- **인증 비밀은 절대 포함하지 않는다**: 두 내보내기 모두 `Session`·
  `LoginToken`·`RecoveryCode`·`Account.totpSecretCiphertext`·
  `Invitation.tokenHash`를 조회 대상에서 아예 뺐다(Prisma `select`로 명시적
  화이트리스트). 관리자 백업이라도 로그인 상태 자체를 복제할 이유는 없고,
  세션 토큰 해시 같은 값이 파일로 굴러다니는 것 자체가 위험이기 때문이다.
- **모든 내보내기는 감사기록에 남는다**: `AuditAction`에 `EXPORT`를 새로 추가했다
  (마이그레이션 `20260921014704_add_export_audit_action`). 관리자 백업은
  `entityType: "Backup", entityId: "ALL"`, 행위자와 테이블별 건수(`afterData`)를
  남기고 `/admin/backup` 화면 자체에서 최근 20건의 내보내기 이력(시각·행위자·
  총 건수)을 바로 보여준다. 본인 내보내기는 `entityType: "Account", entityId:
  <본인 계정 ID>`로 남아 `/admin/audit-log`에서 관리자가 "누가 언제 자기 데이터를
  내려받았는지" 조회할 수 있다.
- **정직하게 남겨둔 것**: 첨부파일은 메타데이터(파일명·크기·업로더 등)만 내보내고
  실제 파일 바이트는 포함하지 않는다 — ADR-0004의 로컬 디스크 저장을 그대로
  압축해 내보내는 것은 별도 작업으로 남겨둔다. Notion 이전 이력
  (`MigrationRun`/`SourceLink`)도 업무 데이터가 아니라 이전 추적용이라 내보내기
  범위에서 뺐다. "다른 운영자로 이전"을 실제로 검증하려면 이 JSON을 다시 읽어
  새 DB로 복원하는 가져오기 기능이 있어야 하는데, 그건 아직 없다 — 지금은
  내보내기(단방향)만 구현했다.
...
- **역할별 홈 화면의 나머지**: 홈 화면에 담당자·운영 역할용 요약 카드는 이제
  있다("내 홈 화면" 절 참고). v0.1 §4.5(사업 책임자 화면의 예산·청구·성과
  탭)·§4.2(사무국 홈의 마감·지연 청구 등)가 그리는 전체 구성은 계약·청구·
  성과 지표 같은 R2 데이터 모델이 없어 옮기지 못했다 — 지금은 이미 있는
  화면(`/review`, `/admin/invitations`, `/needs`)으로 이어주는 요약뿐이다.
...
- **첨부파일 실제 파일의 백업 이관**: S3/MinIO로 옮기는 코드 경로는 이제
  ...
  없다 — 조합이 실제로 버킷을 정하고 켜는 시점에, 그때 쌓인 파일 개수만큼만
  한 번 처리하면 되는 일이라 미리 만들지 않았다.
- **백그라운드 작업 큐(BullMQ/Redis)**: 앱 내 알림(FR-10, `/my/notifications`)은
  이제 있지만, 이메일 등 외부 채널로 재시도·지연 발송하는 실제 작업 큐는 없다 —
...
- **백업 가져오기(복원), 첨부파일 실제 바이트 포함**: 데이터 내보내기(`/admin/
  ...
  메타데이터만 내보내고 실제 파일 바이트는 포함하지 않는다.
...
- 로컬 PostgreSQL 16을 대상으로 `next start` 실행 후 다음을 실제로 확인:
  ...
  - Docker Compose 배포 설정: `output: "standalone"` 추가 후 `npx next build`가
    ...
    "정직하게 남겨둔 것" 참고
  - 데이터 백업·내보내기: 역할 없는 계정과 DOMAIN_OPERATOR만 있는 계정 모두
    `/admin/backup` 화면 접근과 `POST /api/admin/backup/export` 직접 호출이
    `/forbidden`으로 차단됨을 확인; SECRETARIAT 계정으로 전체 내보내기를
    실행하면 실제 DB의 테이블별 건수와 정확히 일치하는 JSON이 `Content-
    Disposition: attachment` 헤더와 함께 내려오고, `Account` 배열에
    `totpSecretCiphertext`가, `Invitation` 배열에 `tokenHash`가 전혀 없음을
    (TOTP가 실제로 활성화된 계정 포함) 직접 확인; 내보내기 직후
    `AuditLog`(entityType `"Backup"`, action `EXPORT`, afterData에 테이블별
    건수)가 남고 `/admin/backup` 화면의 "내보내기 이력"에 시각·행위자·총
    건수로 곧바로 나타남을 확인; `/admin/audit-log`의 종류 드롭다운에도
    `Backup`이 새로 나타남을 확인. 본인 데이터 내보내기는 로그인만 되어
    있으면 역할 없이도 호출 가능함을 확인했고, 받은 JSON에 본인 계정·사람
    정보·본인 기여만 담기고 다른 계정 데이터가 섞이지 않음을 확인; 계정에
    연결된 사람(Subject)이 없는 경우(`subjectId: null`)에도 오류 없이
    `subject: null`과 빈 배열들로 정상 응답함을 확인; 두 경우 모두
    `AuditLog`(entityType `"Account"`, entityId=본인 계정 ID, action
    `EXPORT`)가 정확히 남는 것을 확인; 비로그인 상태로 두 화면·두 API 모두
    호출하면 `/login`으로 리다이렉트됨을 확인
  - 정정 계보 전체 보기(`/activities/[activityId]/lineage`): 원본→1차 정정→
    2차 정정까지 두 단계 정정 체인을 실제로 만든 뒤, 세 버전 중 어느 id로
    들어가도 같은 3개짜리 계보가 원본→1차→2차(최신) 순서로 정확히 표시됨을
    확인; 지금 보고 있는 버전에는 "지금 보는 버전" 표시가 붙고 그 항목만
    링크가 아닌 일반 텍스트로 렌더링됨(자기 자신 링크 없음)을 확인; 활동
    상세 화면에서 세 버전 모두 "정정 계보 전체 보기" 링크가 각자의 id로
    정확히 연결됨을 확인; 정정 이력이 전혀 없는 일반 활동은 상세 화면에
    그 링크 자체가 나타나지 않지만, `/lineage` 경로에 직접 접근하면 자기
    자신 1개짜리 계보로 정상 표시됨(오류 아님)을 확인; 존재하지 않는
    활동 ID로 접근하면 404, 비로그인 상태로 접근하면 `/login`으로
    리다이렉트됨을 확인
  - 로그인 요청·2단계 인증 속도 제한: 같은 이메일로 로그인 링크를 5회 연속
    요청하면 처음 5번은 정상 처리되고 6번째부터 `/login?error=rate_limited`로
    막힘을 확인; 동시에 다른 이메일로 요청하면 전혀 영향받지 않고 정상
    처리됨을 확인(이메일별로 독립된 제한); SYSTEM_ADMIN 계정(TOTP 활성)의
    2단계 인증 코드를 10회 연속 틀리면 그때까지는 매번 "코드가 올바르지
    않습니다"로 정상 실패하다가, 11번째 시도부터는 `rate_limited`로 막히고
    이후에는 **실제로 올바른 코드를 넣어도** 통과되지 않음을 실제 TOTP
    비밀키로 생성한 정확한 코드로 확인(제한이 진짜로 계정을 잠근다는 뜻);
  ...
  - CI(`.github/workflows/ci.yml`): 완전히 새로 만든 PostgreSQL 데이터베이스에
    ...
    `"on":`으로 고쳤다). **후속 업데이트**: 이 저장소를 실제 PR(#2)로 올려
    GitHub Actions에서 두 잡을 처음 돌려본 결과, `docker` 잡이 곧바로 실패를
    잡아냈다 — `runner` 스테이지의 `COPY --from=builder /app/public
    ./public`이 이 저장소에 없는 `public/` 디렉터리를 복사하려 해 빌드
    자체가 깨졌다(이 샌드박스는 Docker Hub 접근이 막혀 있어 로컬에서는 끝내
    발견하지 못했던 버그). 그 COPY 줄을 지우고 다시 푸시하자 `app`·`docker`
    두 잡 모두 성공했다 — Docker Compose 스택이 실제로 빌드·기동·헬스체크를
    통과하는 것을 GitHub Actions에서 처음으로 확인했다.
  - 상담·수요 후속 필요 생성: 활동 운영 역할이 없는 계정은 종결된 상담·수요
    상세 화면에서 "후속 상담·수요 만들기" 링크 자체가 보이지 않고, 역할이
    있는 계정에게는 보임을 확인; 그 링크로 들어간 `/needs/new?
    followsUpOnNeedId=...`에서 선택형이 그 종결 건으로 미리 채워짐을 확인;
    실제로 후속 상담·수요를 접수하면 새 레코드의 상세 화면에 "이 상담·수요는
    [원본]의 후속입니다" 배너가, 원본(종결 건) 상세 화면에는 "후속 상담·수요"
    목록에 상태 배지와 함께 방금 만든 건이 나타남을 확인; 아직 종결되지 않은
    상담·수요를 `followsUpOnNeedId`로 직접 API에 넘기면(화면 선택지에는 애초에
    나타나지 않는 값을 URL로 직접 시도) `error=invalid_follow_up`으로 거부되고
    레코드가 전혀 생성되지 않음을 확인; 존재하지 않는 ID로도 같은 방식으로
    거부됨을 확인; 활동 운영 역할이 없는 계정은 상담·수요 접수 API 자체가
    여전히 `/forbidden`으로 막힘을 확인(회귀 없음)
  - 역할별 홈 화면 요약: 역할도 없고 담당하는 활동·상담도 없는 계정은 "담당자로서
    처리할 일"·"운영 현황" 두 섹션 모두 홈 화면에 나타나지 않음을 확인; 활동을
    관리하고 상담을 담당하는 계정(ACTIVITY_MANAGER+SECRETARIAT)으로 그 활동에
    참여 신청(PROPOSED)·확인 대기 기여(SUBMITTED)를 만들고 그 계정이 담당하는
    상담·수요를 RECEIVED 상태로 두면, "담당자로서 처리할 일"에 세 건수가 정확히
    표시되고 각각 `/review`·`/activities`·`/needs`로 연결됨을 확인; 같은 계정에
    PENDING 초대와 조합 전체의 RECEIVED 상담·수요가 있으면 "운영 현황"에도
    정확한 건수가 표시됨을 확인; SYSTEM_ADMIN 역할만 있고 특정 항목을 직접
    담당하지는 않는 계정도, 실제로 자신이 담당자로 지정된 상담·수요가 있으면
    "담당자로서 처리할 일"이 그 항목만 정확히 반영해 나타남을 확인(역할이
    아니라 실제 담당 여부로 정확히 좁혀짐) — 처리할 항목이 전혀 없으면 역할이
    있어도 그 섹션 자체가 뜨지 않음을 확인
  - 활동 결재 위임: 수정 화면에서 위임 대상을 책임자 자신으로 지정하면
    `error=approver_same_as_manager`, 존재하지 않는 계정 ID를 지정하면
    `error=invalid_approver`로 거부됨을 확인; 정상적으로 위임을 설정한 뒤
    책임자가 승인 요청까지 마치고 그 활동을 스스로 승인·반려하려 하면
    `error=forbidden`으로 차단되고 상태가 그대로 `PENDING_APPROVAL`에
    머무름을 확인; 위임받은 계정은 같은 활동에서 승인·반려 버튼이 실제로
    보이고, 승인을 실행하면 상태가 `PREPARING`으로 바뀌며 `AuditLog`의
    `actorAccountId`가 위임받은 계정으로 정확히 남음을 확인; 위임받은
    계정이 "진행 시작" 같은 승인·반려가 아닌 다른 전이를 시도하면
    `error=forbidden`으로 거부되고 상태가 바뀌지 않음을 확인(위임이 승인·
    반려 두 가지에만 좁게 적용됨); 위임을 걸지 않은 별도 활동에서는 책임자가
    스스로 승인해도 정상적으로 `PREPARING`이 됨을 확인(회귀 없음); 상세
    화면에서 책임자에게는 승인·반려 버튼 대신 "OOO에게 결재를 위임했습니다"
    안내문이, 위임받은 계정에게는 실제 버튼이 보이는 것을 양쪽 계정으로
    직접 확인
  - 계정 상태 관리 화면(`/admin/accounts`, FR-01): 역할 없는 계정은 화면·API
    모두 `/forbidden`으로 막힘을 확인; SECRETARIAT 계정으로 정상 계정을
    정지(사유 없이 시도하면 `error=reason_required`로 거부, 사유를 넣으면
    성공)한 뒤 `AuditLog`에 행위자·이전/이후 상태(`ACTIVE`→`SUSPENDED`)·
    사유가 정확히 남음을 확인; **정지 조치 이전에 이미 발급돼 있던(만료되지
    않은) 그 계정의 세션 쿠키로 보호된 화면에 접근하면 그 즉시 `/login`으로
    막히고, 같은 계정을 재활성화(`reactivate`)하면 같은 쿠키로 다시 접근이
    풀림을 확인**(세션을 따로 폐기하는 코드 없이 `Account.status`만 바꿔도
    되는 이유 — FR-01의 핵심 요건을 이번에 실제 관리자 화면·API 경로로 처음
    검증함, 이전 검증은 DB를 직접 조작한 것이었음); 관리자가 자기 자신의
    계정 상태를 바꾸려 하면 역할과 무관하게 `error=cannot_change_self`로
    거부되고 DB가 바뀌지 않음을 확인; 현재 상태에서 불가능한 전이(예: ACTIVE
    상태에 `reactivate`, WITHDRAWN 상태에 `suspend`)를 시도하면
    `error=invalid_transition`으로 거부됨을 확인; `withdraw`(사유 필수) →
    `rejoin`(사유 불필요) 순환을 실제로 실행해 각 단계의 상태 전이와
    `AuditLog` 기록이 모두 올바름을 확인
