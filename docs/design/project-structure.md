...
- **담당 범위(FR-07의 "담당 범위별 확인 대기 목록")**: 역할 기반 접근 제어가 아직 없어,
  이미 스키마에 있는 소유권 필드(`Activity.managerAccountId`, `Need.assigneeAccountId`)를
  그대로 담당자 판정에 썼다. 정식 권한 시스템이 생기면 이 부분을 교체해야 한다.
...
- **역할 기반 접근 제어(인가)**: 로그인 여부만 확인한다. `/review`·`/admin` 등의 역할별
  제한은 소유권 필드(managerAccountId 등)로 임시 대체했을 뿐, 정식 권한 시스템은 다음 작업.
